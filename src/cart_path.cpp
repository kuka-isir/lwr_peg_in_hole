...
{
  // Initialize ROS
  ros::init(argc, argv, "descartes_path");
  ros::NodeHandle nh;
  ...
  {
//     Eigen::Affine3d pose;
//     pose = Eigen::Translation3d( 0.43, 0.16, 1.53+ 0.01 * i);
//     Eigen::Quaternion q(1.0, 0.0,0.0,0.0);
//     pose *=q;
//     pose.rotate(Eigen::AngleAxisf (theta, Eigen::Vector3f::UnitZ()));
//     descartes_core::TrajectoryPtPtr pt = makeTolerancedCartesianPoint(pose);
//     points.push_back(pt);
    
    geometry_msgs::Pose geom_pose;
    Eigen::Affine3d eigen_pose;
    geom_pose.position.x = 0.367;
    geom_pose.position.y = 0.077;
    geom_pose.position.z = 1.427 + 0.01 *i;
    tf::Quaternion quat = tf::createQuaternionFromRPY(M_PI,0.0,0.0);
    geom_pose.orientation.x = quat.x();
    geom_pose.orientation.y = quat.y();
    geom_pose.orientation.z = quat.z();
    geom_pose.orientation.w = quat.w();
    ROS_INFO("POSE : (%f, %f, %f) (%f, %f, %f, %f)", geom_pose.position.x, geom_pose.position.y, geom_pose.position.z, geom_pose.orientation.x, geom_pose.orientation.y, geom_pose.orientation.z, geom_pose.orientation.w);
    tf::poseMsgToEigen(geom_pose, eigen_pose);
  ...
  const std::string tcp_frame = "link_7";
